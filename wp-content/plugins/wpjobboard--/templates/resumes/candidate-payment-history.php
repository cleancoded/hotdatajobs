...
<div class="wpjb wpjb-page-index">
    ...
    <ul class="wpjb-tabs">
        <li class="wpjb-tab-link <?php if($browse == "all"):?>current<?php endif; ?>">
            <a href="<?php echo wpjr_link_to("mypaymenthistory", null, array("filter"=>"all")) ?>"><?php _e("All", "wpjobboard"); ?></a> (<?php echo $total->all ?>)
        </li>
        <li class="wpjb-tab-link <?php if($browse == "completed"):?>current<?php endif; ?>">
            <a href="<?php echo wpjr_link_to("mypaymenthistory", null, array("filter"=>"completed")) ?>"><?php _e("Completed", "wpjobboard"); ?></a> (<?php echo $total->completed ?>)
        </li>
        <li class="wpjb-tab-link <?php if($browse == "pending"):?>current<?php endif; ?>">
            <a href="<?php echo wpjr_link_to("mypaymenthistory", null, array("filter"=>"pending")) ?>"><?php _e("Pending", "wpjobboard"); ?></a> (<?php echo $total->pending ?>)
        </li>
        <li class="wpjb-tab-link <?php if($browse == "failed"):?>current<?php endif; ?>">
            <a href="<?php echo wpjr_link_to("mypaymenthistory", null, array("filter"=>"failed")) ?>"><?php _e("Failed", "wpjobboard"); ?></a> (<?php echo $total->failed ?>)
        </li>
        <li class="wpjb-tab-link <?php if($browse == "refunded"):?>current<?php endif; ?>">
            <a href="<?php echo wpjr_link_to("mypaymenthistory", null, array("filter"=>"refunded")) ?>"><?php _e("Refunded", "wpjobboard"); ?></a> (<?php echo $total->refunded ?>)
        </li>
    </ul>
    ...
    <div class="wpjb-grid wpjb-tab-content">
        ...
        <?php include $this->getTemplate("resumes", "payment-history-item") ?>
        <?php endforeach; else :?>
